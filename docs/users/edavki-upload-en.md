...
> * **FIFO Rule**: FURS requires the "First-In, First-Out" method. Our app generates data based strictly on your Trade Republic report.
> * **Multiple Brokers**: If you trade the **same ISIN** on another broker (e.g., Revolut or IBKR), you cannot simply upload this file. You must manually consolidate the data to ensure the FIFO sequence is correct across all accounts.
> * **Parser Logic**: We use the "Gains and Losses" section from Trade Republic where the broker has already paired buy/sell amounts. This works perfectly if you only trade that ISIN on Trade Republic.
